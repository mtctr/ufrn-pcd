...
#  endif
   ...
   Read_vector(local_x, local_n, n, "x", my_rank, type, comm);
   Print_vector(local_x, local_n, n, "x is", my_rank, type, comm);
   Read_vector(local_y, local_n, n, "y", my_rank, type, comm);
   Print_vector(local_y, local_n, n, "y is", my_rank, type, comm);

   Parallel_vector_sum(local_x, local_y, local_z, local_n);
   Print_vector(local_z, local_n, n, "The sum is", my_rank, type, comm);
...
void Read_vector(
   ...
   if (my_rank == 0) {
      ...
      for (i = 0; i < n; i++)
         scanf("%lf", &a[i]);
      MPI_Scatter(a, 1, type, local_a, 1, type, 0,
         comm);
      free(a);
   } else {
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
...
void Print_vector(
   ...
   if (my_rank == 0) {
      ...
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
            comm);
      MPI_Gather(local_b, 1, type, b, 1, type,
            0, comm);
      printf("%s\n", title);
      for (i = 0; i < n; i++)
         printf("%f ", b[i]);
      ...
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
            comm);
      MPI_Gather(local_b, 1, type, b, 1, type, 0,
         comm);
   }
}  /* Print_vector */
