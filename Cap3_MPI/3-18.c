...
void Read_vector(double local_a[], int local_n, int n, char vec_name[],
      int my_rank, int comm_sz, MPI_Datatype type, MPI_Comm comm);
...
#  endif
   ...
   MPI_Datatype type;
   MPI_Type_vector(comm_sz,local_n,local_n, MPI_DOUBLE, &type);
   MPI_Type_commit(&type);
   Read_vector(local_x, local_n, n, "x", my_rank, comm_sz, type, comm);
   Print_vector(local_x, local_n, n, "x is", my_rank, comm_sz, type, comm);
   Read_vector(local_y, local_n, n, "y", my_rank, comm_sz, type, comm);
   Print_vector(local_y, local_n, n, "y is", my_rank, comm_sz, type, comm);

   Parallel_vector_sum(local_x, local_y, local_z, local_n);
   Print_vector(local_z, local_n, n, "The sum is", my_rank, comm_sz, type, comm);
...
void Read_vector(
   ...
   if (my_rank == 0) {
     ...
     for (i = 1; i < comm_sz; i++){
       MPI_Send(a, 1, type, i, 0, comm);
       }
       free(a);
     } else {
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
            comm);
      MPI_Recv(&a, 1, type, 0, 0, comm,
            MPI_STATUS_IGNORE);
   }
}  /* Read_vector */
...
void Print_vector(
   ...
   if (my_rank == 0) {
      ...
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
            comm);
      for(i = 1; i<comm_sz; i++){
        MPI_Recv(&b, 1, type, i, 1, comm, MPI_STATUS_IGNORE);
      }
      printf("%s\n", title);
      for (i = 0; i < n; i++){
         printf("%f ", b[i]);
